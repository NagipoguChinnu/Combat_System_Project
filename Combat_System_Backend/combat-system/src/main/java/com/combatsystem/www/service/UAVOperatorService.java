...
public class UAVOperatorService {
    ...
    public UserResponseDto getUavByEmail(String email) {
        return uavRepository.findFirstByEmail(email)
    ...
    public UserResponseDto updateUav(UserResponseDto dto) {
        Optional<UAVOperator> uavOpt = uavRepository.findFirstByEmail(dto.getEmail());

        if (uavOpt.isEmpty()) {
            return null; // Or throw custom exception
        }

        UAVOperator uav = uavOpt.get();
        uav.setUserName(dto.getName());
        uav.setDeviceType(dto.getRole());
        uav.setLatitude(dto.getLatitude());
        uav.setLongitude(dto.getLongitude());
        uav.setPassword(dto.getPassword());

        UAVOperator updated = uavRepository.save(uav);
