...
public class IOTOperatorService {
    ...
    public List<IOTOperatorDto> getPendingIOTOperators() {
        List<IOTOperator> approved = iotRepository.findByStatus("PENDING");
        return approved.stream()
            .map(op -> new IOTOperatorDto(
                op.getId(),
                op.getUserName(),
                op.getDeviceType(),
                op.getLatitude(),
    ...
    public List<IOTOperatorDto> getDeniedIOTOperators() {
        List<IOTOperator> approved = iotRepository.findByStatus("DENIED");
        return approved.stream()
            .map(op -> new IOTOperatorDto(
                op.getId(),
                op.getUserName(),
    ...
    public List<IOTOperatorDto> getApprovedIOTOperators() {
        List<IOTOperator> approved = iotRepository.findByStatus("APPROVED");
        return approved.stream()
            .map(op -> new IOTOperatorDto(
                op.getId(),
                op.getUserName(),
    ...
    public String login(String email, String password, String otp) {
        ...
        switch (iotOperator.getStatus()) {
            ...
            case "BLOCKED":
            	return "Your account was blocked due to multiple invalid OTP attempts";
            case "APPROVED":
    ...
    public IOTOperatorDto getIotByEmail(String email) {
        return iotRepository.findFirstByEmail(email)
            .map(s -> new IOTOperatorDto(
                s.getUserName(),
                s.getEmail(),
                s.getDeviceType(),
                s.getLatitude(),
    ...
    public IOTOperatorDto updateIot(IOTOperatorDto dto) {
        Optional<IOTOperator> iotOpt = iotRepository.findFirstByEmail(dto.getEmail());

        if (iotOpt.isEmpty()) {
            return null; // Or throw custom exception
        }

        IOTOperator iot = iotOpt.get();
        iot.setUserName(dto.getUserName());
        iot.setDeviceType(dto.getDeviceType());
        iot.setLatitude(dto.getLatitude());
        iot.setLongitude(dto.getLongitude());
        iot.setPassword(dto.getPassword());

        IOTOperator updated = iotRepository.save(iot);

        return new IOTOperatorDto(
                updated.getUserName(),
                updated.getEmail(),
